<?php
	...
	if(isset($_GET["fn"])){
		...
		switch($_GET["fn"]){
			case "login" :
					$userdata = $DB->GetDataOneRow("SELECT `AdminID`, `adGender`, `adLevel`, `adUnder`, `adEform`, `adRegion`, `adProvince`, `adArea`, `adBranch`, `adUsername`, `adPassword`, `adFullname`, `pvName`, `arName`, `brName` FROM `Admin`,`Province`,`Area`, `Branch` WHERE adProvince = ProvinceID AND AreaID = adArea AND BranchID = adBranch AND adStatus = 1 AND adUsername = ? AND adPassword = BINARY ?",array("ss",$_GET["user"],$_GET["password"]));
					...
					if($user->Level < 2){
						switch($user->Under){
							case 0 ://ปรับแต่งได้ทั้งหมด
								break;
							case 1 ://ภาค
									if(($user->Region <> $_GET["UnderCode"]) || ($_GET["Under"] <> 1)){
								...
								break;
							case 2 ://พื้นที่
									if(($user->Area <> $_GET["UnderCode"]) || ($_GET["Under"] <> 2)){
										$user->Message = "ไม่สามารถแก้ไขข้อมูลของบุคลากรนอกสังกัดได้";
									}else{
										$cansave = true;
									}
								break;
							case 3 ://สาขา
									if(($user->Branch <> $_GET["UnderCode"]) || ($_GET["Under"] <> 2)){
										$user->Message = "ไม่สามารถแก้ไขข้อมูลของบุคลากรนอกสังกัดได้";
									}else{
										$cansave = true;
									}
						...
						if($cansave){
							$data = array(
								"adLevel" => $_GET["Level"],
								"adEform" => $_GET["Eform"],
								"adFullname" => $_GET["Fullname"],
								"adEmail" => $_GET["Email"],
								"adTel" => $_GET["Tel"],
								"adMobile" => $_GET["Mobile"],
								"adStatus" => $_GET["Status"],
							);
							if(isset($_GET["Password"]) && ($_GET["Password"]!="")){
								$data["adPassword"] = $_GET["Password"];
							}
							if($_GET["id"] > 0){
								switch($user->Under){
									case 0 ://ปรับแต่งได้ทั้งหมด
										break;
									case 1 ://ภาค
										$DB->UpdateData("Admin",$data,"adUnder = ? AND adRegion = ? AND AdminID = ?",array("iii",$user->Under,$user->Region,$_GET["id"]));
										break;
									case 2 ://พื้นที่
										$DB->UpdateData("Admin",$data,"adUnder = ? AND adArea = ? AND AdminID = ?",array("iii",$user->Under,$user->Area,$_GET["id"]));
										break;
									case 3 ://สาขา
										$DB->UpdateData("Admin",$data,"adUnder = ? AND adBranch = ? AND AdminID = ?",array("iii",$user->Under,$user->Branch,$_GET["id"]));
										break;
								}
							}else{
								$data["AdminID"] = $DB->NextID("Admin");
								$data["adGender"] = $_GET["Gender"];
								$data["adUsername"] = $_GET["Username"];
						...
						if(isset($_GET["Password"]) && ($_GET["Password"]!="")){
							$DB->UpdateData("Admin",array("adPassword" => $_GET["Password"]),"AdminID = ?",array("i",$user->id));
							if($DB->GetAffectRows() > 0){
								$user->Message = "พบข้อผิดพลาด ไม่สามารถบันทึกข้อมูลได้";
							}else{
								$user->Message = "บันทึกข้อมูลสำเร็จ";
			...
			case "gettable" :
				...
				break;
			case "getdata" :
					$userdata = $DB->GetDataOneRow("SELECT `AdminID`, `adGender`, `adLevel`, `adUnder`, `adEform`, `adRegion`, `adProvince`, `adArea`, `adBranch`, `adUsername`, `adFullname`, `pvName`, `arName`, `brName`, `adStatus`, `adEmail`, `adTel`, `adMobile` FROM `Admin`,`Province`,`Area`, `Branch` WHERE adProvince = ProvinceID AND AreaID = adArea AND BranchID = adBranch AND AdminID = ?",array("i",$_GET["id"]));
